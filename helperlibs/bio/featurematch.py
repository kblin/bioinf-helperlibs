    ...
    for seq_i in seqs:
        for feature in seq_i.features:
            if feature.type == "CDS" and (locus_tag == "all" or
                ...
                start = max(0, int(feature.location.start) - utr_len)
                stop = max(0, int(feature.location.end) + utr_len)
                feature_seq = seq_i.seq[start:stop]
                f_match = FeatureMatch(feature, feature_seq, feature.location.strand,
                                       utr_len)
